...
def compute_binary_metrics(
    ...
    if curr_feature_columns is None:
    ...
    if train_data is not None:
        metrics_str += f'Число объектов на обучении: {train_data.shape[0]:,}\n'
    if val_data is not None:
        metrics_str += f'Число объектов на валидации: {val_data.shape[0]:,}\n'
    if test_data is not None:
        metrics_str += f'Число объектов на тесте: {test_data.shape[0]:,}\n'
    metrics_str += '\n'
